    ...
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      {!isEmailSent && <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          ...
          <input type="email" placeholder='Email id' onChange={e => setEmail(e.target.value)} value={email} className='bg-transparent outline-none text-white' required/>
        </div>
      ...
      {!isOtpSubmited && isEmailSent && <form onSubmit={onSubmitOtp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='flex justify-between mb-8' onPaste={handlePaste}>
          {Array(6).fill(0).map((_, index) => (
            <input className='w-12 h-12 bg-[#333A5C] text-white text-center text-xl rounded-md' type="text" maxLength='1' key={index}
              ref={e => inputRefs.current[index] = e} onInput={(e) => handleInput(e, index)} onKeyDown={(e) => handleKeyDown(e, index)} required/>
          ))}
        </div>
      ...
      {isOtpSubmited && isEmailSent && <form onSubmit={onSubmitNewPassword} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          ...
          <input type="password" placeholder='Password' onChange={e => setNewPassword(e.target.value)} required value={newPassword} className='bg-transparent outline-none text-white' />
        </div>
        <button className='w-full py-3 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full'>Submit</button>
      </form>}
    </div>
